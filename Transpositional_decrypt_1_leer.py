...
def DoReverseCipher():
    ...
    plain = encryptionFile.reverse(cipher, (KeepBlanks.get() == "1"), (KeepNonalpha.get() == "1"))
...
def SkytaleDiameterChanged():
    cipher = PrepareForDecryption()
    plain = encryptionFile.skytaleDecryption(cipher, int(SpinboxSkytaleDiameter.get()))
    explanation = encryptionFile.skytaleDecriptionExplination(cipher, int(SpinboxSkytaleDiameter.get()), '')
    TextPlain.insert("1.0", plain)
