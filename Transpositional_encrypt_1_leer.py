...
def DoReverseCipher():
    ...
    cipher = encryptionFile.reverse(plain, (KeepBlanks.get() == "1"), (KeepNonalpha.get() == "1"))
...
def SkytaleDiameterChanged():
    plain = PrepareForEncryption()
    cipher = encryptionFile.skytale(plain, int(SpinboxSkytaleDiameter.get()))
    explanation = encryptionFile.skytaleExplination(plain, int(SpinboxSkytaleDiameter.get()), '')
    #explanation = encryptionFile.skytaleExplinationWithoutSpaces(plain, int(SpinboxSkytaleDiameter.get()))
    TextCiph.insert("1.0", cipher)
